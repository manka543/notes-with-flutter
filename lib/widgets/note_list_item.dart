...
class NoteListItem extends StatefulWidget {
  const NoteListItem(
      {super.key,
      required this.item,
      required this.changeItemProgress,
        ...
        child: Row(
          children: [
            IconButton(
              ...
              icon: Icon(
                  widget.item.done ? Icons.priority_high_outlined : Icons.done),
            ),
            Expanded(
                child: AnimatedDefaultTextStyle(
                    style: widget.item.done
                        ? const TextStyle()
                        : const TextStyle(
                            decoration: TextDecoration.lineThrough),
                    duration: const Duration(milliseconds: 500),
                    child: Text(widget.item.text ?? "There is no text"))),
            IconButton(
                onPressed: () {
